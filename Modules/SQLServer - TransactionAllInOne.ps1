...
function UpdateTable {
    ...
    if ($totalQueries -gt 0) {
        ...
        # Exécuter toutes les requêtes en une seule transaction si apply = "yes"
        if ($apply -eq "yes") {
            $transaction = $connection.BeginTransaction()
            
            try {
                $combinedQuery = $allQueries -join "`n"
                $command = $connection.CreateCommand()
                ...
                # Afficher toutes les requêtes après exécution réussie
                foreach ($query in $allQueries) {
                    MOD "UpdateTable" "SQL : $query"
                }
                LOG "UpdateTable" "Exécution réussie de $($allQueries.Count) requêtes en une seule transaction"
            } catch {
                $transaction.Rollback()
                ERR "UpdateTable" "Échec de l'exécution des requêtes combinées, transaction annulée: $($_.Exception.Message)" -ADDERR
            }
        } else {
            # Afficher les requêtes sans les exécuter
            foreach ($query in $allQueries) {
